...
- **Semantic HTML5** — every page uses `<header>`, `<nav>`, `<main>`, `<article>`, `<section>`, `<aside>`, `<time>` and `<footer>` appropriately; `<div>`/`<span>` are reserved for pure layout. Examples: [`AppShell.tsx`](frontend/src/components/layout/AppShell.tsx), [`BibleTalkChat.tsx`](frontend/src/pages/BibleTalkChat.tsx) (each message is an `<article>`), [`Dashboard.tsx`](frontend/src/pages/Dashboard.tsx).
- **Two interactive APIs (must be ≥ 2)** — both used meaningfully, not as decoration:
  1. **HTML5 Drag and Drop** — `WeeklyCalendar` ([`components/WeeklyCalendar.tsx`](frontend/src/components/calendar/WeeklyCalendar.tsx)) lets you drag a study card to a new day/hour. Native `onDragStart`/`onDrop`, `dataTransfer.setData("text/plain", studyId)`, writes the new `scheduledAt` back to Firestore. Keyboard fallback: focus the study card and press Shift + arrow keys (this also satisfies the accessibility item).
  2. **HTML5 Canvas 2D** — `WeeklyHeatmapCanvas` ([`components/WeeklyHeatmapCanvas.tsx`](frontend/src/components/charts/WeeklyHeatmapCanvas.tsx)) hand-draws the dashboard heatmap pixel-by-pixel via `CanvasRenderingContext2D`. Cell **hue** shifts cool-to-warm by time of day, **opacity** scales with study count, and inline `M`/`F` glyphs show gender breakdown. Pointer hit-testing drives a tooltip listing each study in the slot. The canvas is mirrored by a `sr-only-table` so screen-reader users get the same data.
- **Responsive at 320/768/1024** — Tailwind responsive prefixes; the desktop top-nav collapses to a bottom-tab nav on mobile (`md:hidden` + `hidden md:flex` in [`AppShell.tsx`](frontend/src/components/layout/AppShell.tsx)). Calendar, chat, and dashboard all reflow to single-column at 320px. Verified manually in Chrome DevTools device toolbar.
- **CSS framework** — Tailwind CSS only ([`tailwind.config.js`](frontend/tailwind.config.js), [`postcss.config.js`](frontend/postcss.config.js), reusable component classes in [`src/styles/index.css`](frontend/src/styles/index.css)). No vanilla CSS files, no styled-components, no other CSS framework.
- **SPA** — React Router v6 ([`App.tsx`](frontend/src/App.tsx)). Every navigation is client-side; nginx's `try_files $uri $uri/ /index.html` fallback ([`nginx.conf`](frontend/nginx.conf)) means deep links also serve the SPA shell. Routes are code-split with `React.lazy` + `Suspense` (per-route chunks), the title updates per view, unknown routes render a real `NotFound` 404 page (not a silent redirect), and a top-level [`ErrorBoundary`](frontend/src/components/layout/ErrorBoundary.tsx) catches render errors instead of showing a blank screen.
...
- **App-shell visible offline** — `NetworkFirst` strategy with a 3-second network timeout falls back to the precached `index.html`; an [`OfflineBanner`](frontend/src/components/layout/OfflineBanner.tsx) explains the state.
- **Queued database inserts** — [`offlineQueue.ts`](frontend/src/store/offlineQueue.ts) backs offline chat sends and study edits with IndexedDB. The `online` event flushes the queue via [`queueProcessor.ts`](frontend/src/store/queueProcessor.ts). This is exactly the example the spec calls out ("queued database inserts").
- **Server-initiated notifications (two channels)** — the spec accepts any one of Web Push / WebSockets / SSE; we ship two:
  1. **Web Push** via Firebase Cloud Messaging. The browser requests `Notification` permission and registers an FCM token in [`store/push.ts`](frontend/src/store/push.ts) using `VITE_FIREBASE_VAPID_KEY` (the Web Push VAPID public key); the token is persisted to `users/{uid}.fcmTokens` by [`routes/notify.ts`](backend/src/routes/notify.ts). The service-worker `push` + `notificationclick` handlers live in [`src/sw.ts`](frontend/src/sw.ts). Backend senders: the ministry-leader-only "Send test push" button (`POST /api/notify/test`) and the reminder [`services/scheduler.ts`](backend/src/services/scheduler.ts).
  2. **Server-Sent Events** as the foreground complement, so a live server push is visible even without OS notification permission. `GET /api/notify`-adjacent `GET /api/stream` ([`routes/stream.ts`](backend/src/routes/stream.ts)) opens a role-scoped `text/event-stream`; the hub is [`services/sse.ts`](backend/src/services/sse.ts). Both the test-push endpoint and the reminder scheduler `broadcast()` over it. Client subscriber: [`store/sse.ts`](frontend/src/store/sse.ts), wired in [`App.tsx`](frontend/src/App.tsx). Because `EventSource` cannot send an `Authorization` header, the stream verifies a Firebase ID token passed on the query string and is mounted before the global Bearer middleware.
- **HTTPS** — GKE Ingress + Google-managed certificate ([`k8s/ingress.yaml`](k8s/ingress.yaml)).

## Authentication and Security

- **Auth** — Firebase Authentication with Google sign-in (hashing-by-proxy clause). Frontend uses `signInWithPopup` ([`store/firebase.ts`](frontend/src/store/firebase.ts)). Backend verifies every `Authorization: Bearer <Firebase ID token>` on every `/api/*` call via Admin SDK ([`middleware/verifyFirebaseToken.ts`](backend/src/middleware/verifyFirebaseToken.ts)).
- **Mitigations**
  - **XSS** — React auto-escapes by default. An ESLint rule ([`.eslintrc.cjs`](frontend/.eslintrc.cjs)) bans `dangerouslySetInnerHTML`. nginx ships a strict **Content-Security-Policy** (`default-src 'self'`, `object-src 'none'`, `frame-ancestors 'none'`, scripts locked to same-origin + the Google API host the Firebase Auth popup needs) alongside `X-Content-Type-Options nosniff`, `X-Frame-Options DENY`, `Referrer-Policy strict-origin-when-cross-origin`, and a `Permissions-Policy` denying camera/mic/geolocation ([`nginx.conf`](frontend/nginx.conf)). The API itself ships `default-src 'none'` via Helmet ([`server.ts`](backend/src/server.ts)).
  - **Injection** — every `/api` body is parsed and validated by Zod schemas ([`routes/ai.ts`](backend/src/routes/ai.ts), [`routes/notify.ts`](backend/src/routes/notify.ts), [`routes/stream.ts`](backend/src/routes/stream.ts)). Firestore SDK queries are parameterised by their typed API — no string concatenation, no SQL surface. Gemini's `/api/ai/ask` cannot widen visibility because the backend force-injects role-scope into every function-call tool invocation ([`services/firestoreTools.ts`](backend/src/services/firestoreTools.ts)).
  - **CSRF** — the API uses bearer tokens, not cookies, so there is no ambient credential for a third-party site to ride. As defense-in-depth we also enforce an explicit **same-origin guard** on every state-changing `/api` request ([`server.ts`](backend/src/server.ts)): a `Sec-Fetch-Site: cross-site` header or an `Origin` outside the CORS allowlist is rejected with `403` before reaching a handler.
- **Transport** — `Strict-Transport-Security` (HSTS, 1-year `max-age` + `includeSubDomains`) set by nginx for the SPA and by Helmet for the API.
- **Helmet** — global `helmet()` middleware with an explicit CSP ([`server.ts`](backend/src/server.ts)).
- **Rate limiting** — `express-rate-limit` at 60 req/min/IP on `/api/*`.
- **Dependency hygiene** — `npm audit` is clean of production-shipping and critical advisories; an `overrides` pin in the root [`package.json`](package.json) forces a patched `shell-quote`. The only residual advisories are dev-only build tooling (`vite`/`vitest`/`esbuild`) that never ship in the nginx/Node images.
...
- **Write integrity** — multi-document writes are atomic: onboarding uses a `writeBatch` and study completion uses a `runTransaction` (read invitee, then update study + progress together). AskBot queries are bounded with `limit()` and use server-side `count()`. See [`DATABASE.md`](DATABASE.md) "ODM, validation, and write patterns".
- **Billing alerts** — set up in the GCP console at $5 / $10 / $20 thresholds (see [`SETUP.md`](docs/SETUP.md)). The spec explicitly requires this for Firebase users.
