...
## ODM, validation, and write patterns

- **ODM** — the spec explicitly accepts the Firestore SDK as the document ODM. The frontend uses the Firebase Web SDK (`firebase/firestore`) and the backend uses the Firebase Admin SDK (`firebase-admin/firestore`). All collection/document access goes through the typed SDK API (`collection`, `doc`, `where`, `arrayUnion`, …) — there is no string-built query and no raw query surface anywhere in the codebase.
- **Input validation** — every write that originates from a backend `/api` route is validated by a Zod schema before it touches Firestore: [`routes/ai.ts`](backend/src/routes/ai.ts), [`routes/notify.ts`](backend/src/routes/notify.ts), [`routes/stream.ts`](backend/src/routes/stream.ts), [`routes/dev.ts`](backend/src/routes/dev.ts). Client-originated writes are additionally constrained by the Firestore security rules below.
- **Atomic writes** — onboarding writes two documents (the user's scope fields and the bible talk's `memberIds`) in a single `writeBatch` so they commit atomically ([`useCompleteOnboarding.ts`](frontend/src/hooks/mutations/useCompleteOnboarding.ts)). This avoids the partial state where a user appears in `memberIds` without their `campusId`/`bibleTalkId` set.
- **Transactions** — marking a study complete runs in a `runTransaction` ([`useCompleteStudy.ts`](frontend/src/hooks/mutations/useCompleteStudy.ts)): it reads the invitee document, then atomically sets `studies/{id}.status = "completed"` and advances `invitees/{id}.currentStudyIndex` only if this study is further along than the freshly-read value. The read-before-write transaction prevents a stale client cache from regressing an invitee's progress.
- **Bounded queries** — every AskBot Firestore query is capped with `limit()` (`listStudies` 100, `listInvitees` 200) and aggregate counts use Firestore's server-side `count()` rather than reading whole collections ([`firestoreTools.ts`](backend/src/services/firestoreTools.ts)).
- **Idempotent server writes** — the reminder scheduler flips `studies/{id}.reminderSent` after sending; because single-document writes are atomic in Firestore, the first replica to flip it wins and duplicate ticks across replicas never double-send ([`services/scheduler.ts`](backend/src/services/scheduler.ts)).
