...
- **Offline queue** — [`src/store/offlineQueue.ts`](frontend/src/store/offlineQueue.ts) wraps `idb-keyval`. When `navigator.onLine === false`, chat messages and study edits are queued; an `online` event handler flushes them via [`queueProcessor.ts`](frontend/src/store/queueProcessor.ts).
- **Canvas heatmap** — [`src/components/charts/WeeklyHeatmapCanvas.tsx`](frontend/src/components/charts/WeeklyHeatmapCanvas.tsx) hand-draws a 7-day × 14-hour grid on a `<canvas>` with HSL fills (hue interpolated by time of day, saturation by study count) and pointer-hit-test tooltips. A `<table class="sr-only-table">` mirror exposes the same data to screen readers.
- **Drag and drop** — [`src/components/calendar/WeeklyCalendar.tsx`](frontend/src/components/calendar/WeeklyCalendar.tsx) uses native HTML5 D&D APIs (`onDragStart`, `onDrop`, `dataTransfer`). Each cell is a drop target; dropping a study writes a new `scheduledAt` to Firestore. Keyboard fallback: Shift + arrow keys.
